...
public class WMTIAnalysis {
    ...
    public Map<String, Object> getAnalysisByWMTICode(String wmtiCode) {
        ...
        try {
            ...
            // JSON 파일을 Map으로 변환
            Map<String, Map<String, Object>> data = objectMapper.readValue(inputStream, Map.class);

            // 성향 코드에 맞는 데이터가 있으면 반환
            if (data.containsKey(wmtiCode)) {
                analysisResult = data.get(wmtiCode);
            } else {
                // 예외 처리 (없는 코드가 들어오면 메시지 반환)
                analysisResult = Map.of("message", "No analysis found for code: " + wmtiCode);
            }
        } catch (Exception e) {
            e.printStackTrace();
            analysisResult = Map.of("message", "Error reading analysis data.");
        }
